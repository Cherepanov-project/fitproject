...
const SocialLoginBtn: FC<ISocialLoginBtn> = ({ redirectURL, typeOfSocial, img }) => {
  ...
  const loginWithSocial = async () => {
    ...
    const responseURLCode = queryString.parse(window.location.search).access_token;

    if (typeOfSocialData.state === typeOfSocial) {
      const token = await loginUserWithSocials(responseURLCode);
      await Cookies.set('userLogin', JSON.stringify({ type: typeOfSocial, access: token }), {
        expires: 2,
      });
